...
public class Game {
	...
	public void targetCity(Army army, String targetName) throws AlreadyTargetedException{
		int i;
		for (i = 0;i<this.getAvailableCities().size();i++){
			if(this.getAvailableCities().get(i).getName().equals(targetName))
				break;
		}
		if(this.getAvailableCities().get(i).isTargeted())
			throw new AlreadyTargetedException();
		
		this.getAvailableCities().get(i).setTargeted(true);
	...
	public void endTurn() {
		...
		for (Army a : player.getControlledArmies()) {
			...
			if (a.getDistancetoTarget() == 0) {
				...
				int i;
				for(i=0;i<this.getAvailableCities().size();i++){
					if(this.getAvailableCities().get(i).getName().equals(a.getCurrentLocation()))
						break;
				}
				if(!(this.getAvailableCities().size()==i))
					this.getAvailableCities().get(i).setTargeted(false);
				a.setTarget("");
